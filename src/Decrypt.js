  ...
  async function addxDaiToMetamask() {
    if (typeof window !== 'undefined') {
      window.ethereum.request({
        ...
        alert("Error Swtiching to Gnosis Chain (xDai)")
        return
  ...
  async function handleSubmitdecrypt() {
    ...
    const fileresponse = fetch(url)
      ...
      .then(function (myBlob) {
        file1 = new File([myBlob], "document.zip", { type: fileresponse.type });
      })
      .catch(rejected => {
        ...
        >
          ...
          <Grid item xs={2} sm={6}>
            ...
            <Grid item>
              ...
              <Button
                onClick={() => { addxDaiToMetamask();setDoc('document'); handleSubmitdecrypt() }}
                variant='primary'
              >
                <label for="button">
